...
En `Hebri-AI-Harness 0.16.0`, el chat visible actúa como **intérprete** por
...
Subgates P0 y condicionales vigentes en `0.16.0`:
...
role-contracts/<rol>.yaml
security-profiles/<perfil>.yaml
runtime-profiles/<rol>-runtime.yaml
context-packs/<rol>-context.yaml
tool-packs/<rol>-tools.yaml
playbooks/<rol>-playbook.yaml
failure-modes/<rol>-failure-modes.yaml
evaluation-rubrics/<rol>-quality.yaml
handoff-contracts/<origen>-to-<destino>.yaml
```
...
```text
HL0_agent_authority:
La IA no define agentes, roles, permisos, capabilities ni escalaciones.
...
- si falta `role-contracts/<rol>.yaml`, no se instancia;
- si falta security profile, runtime profile, context pack, tool pack,
  playbook o rubric, el agente no queda completo;
...
Responsabilidades vigentes desde 0.8.10 y consolidadas en `0.16.0`:
...
- `leader`: valida `agent-registry.yaml`, contratos de rol y lifecycle antes
  de instanciar agentes.
- `implementer`/`worker`: sólo escriben con capability y scope aprobados.
- `reviewer`: puede aprobar o bloquear, pero no editar ni revisar su propio
  output.
...
En harness `0.16.0`, `spec_author` e `implementer` pueden verse como perfiles
...
## Detractor Senior, Reporter y Roles Derivados en 0.16.0
...
En 0.16.0, los prompts de roles quedan subordinados al Agent Contract System.
...
del harness, se trata como no instanciable hasta regularizarlo.

La fuente humana de cada rol es `agents/<rol>.md`. Desde ahí el harness genera
capas derivadas:
...
Los derivados no se editan a mano. Si un contrato YAML, prompt de rol o default
de capability cambia sin modificar `agents/<rol>.md`, el drift-check del
instruction builder debe fallar. El camino correcto es editar la fuente del rol
y regenerar con `scripts/build-instructions.ps1 -WriteOutputs`.

La autosuficiencia del agente no significa autonomía ilimitada. Significa que
el harness le entrega el contexto, herramientas, playbook, fallos esperables,
...
En 0.16.0, si el rol opera vía MCP, aplica la misma regla: `run_command` no
saltea permisos, `preflight_approve` sólo materializa un `SI` explícito y
`close_cycle_check` bloquea cierres sin evidencia, locks/handoffs resueltos y
estado coherente. `session_usage` puede informar consumo/costo y ahorro de
contexto, pero no cambia permisos, roles ni veredictos.

En 0.16.0, `agent_audit` y `agent_review` pueden pedir una auditoria o review
por MCP, y Claude puede tener agentes nativos en `.claude/agents/`. Eso no
convierte al host en autoridad. El harness sigue definiendo:

- que roles existen;
- que backend puede usarse;
- que tools son read-only o con efecto;
- que evidencia minima debe volver;
- que fallback aplica cuando el host no soporta subagentes reales.

Si un archivo nativo de host contradice `agent-registry.yaml`,
`agents/<rol>.md` o los contratos derivados, gana el harness y el drift debe
bloquearse antes de cerrar.
